...
def install_numpy_pickle_compat_shims() -> None:
    """Install aliases for legacy numpy module paths used in pickled SB3 data.

    Some older saved models reference private numpy modules under `numpy._core.*`
    that are no longer importable in newer/older numpy mixes (e.g.
    `numpy._core.numeric`). Mapping these to `numpy.core.*` is sufficient for
    cloudpickle unpickling of SB3 model metadata.
    """
    try:
    ...
    def _compat_bit_generator_ctor(bit_generator_name="MT19937"):
        ...
        # Fall back to original behavior for unknown cases.
